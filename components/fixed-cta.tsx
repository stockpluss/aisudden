...
export function FixedCTA() {
  ...
  const formatPhoneNumber = (value: string) => {
    ...
    if (numbers.length <= 3) {
      ...
      // 010-123-4 형태 (중간자리가 3자리)
      return `${numbers.slice(0, 3)}-${numbers.slice(3, 6)}-${numbers.slice(6)}`
    } else if (numbers.length <= 10) {
      // 010-1234-5678 형태 (중간자리가 4자리)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error("[v0] Submission error:", result.error)
      }
    } catch (error) {
      alert("접수 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.")
  ...
  return (
    <>
      <div
        id="cta"
        className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-r from-primary via-primary to-secondary shadow-2xl border-t-2 border-secondary"
      >
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      disabled={isSubmitting}
                    />
                    <label htmlFor="privacy" className="text-xs sm:text-sm text-primary-foreground/90 cursor-pointer">
                      개인정보 수집 및 이용에 동의합니다{" "}
                      <button
                        type="button"
                        onClick={(e) => {
                  ...
                  <a href="https://s-tockplus.com" target="_blank" rel="noopener noreferrer" className="shrink-0">
                    <img src="/images/horizontal-logo.png" alt="Stock Plus" className="h-5 sm:h-6 w-auto" />
                  </a>
                </div>
              </form>
            )}
        ...
        <DialogContent className="max-w-3xl max-h-[80vh]">
          ...
          <ScrollArea className="h-[60vh] pr-4">
            <div className="space-y-4 text-sm">
              ...
              <p>
                스탁플러스(이하 "회사"라 함)는 이용자의 개인정보를 소중히 여기며, 『개인정보 보호법』 등 관련 법령을
